...
let handler = async(m, { conn }) => {
	...
	fkontak = { key: {participant: `0@s.whatsapp.net`, ...(m.chat ? { remoteJid: `status@broadcast` } : {}) }, message: { 'contactMessage': { 'displayName': wm, 'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;${wm},;;;\nFN:${wm},\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabell:Ponsel\nEND:VCARD`, 'jpegThumbnail': require('fs').readFileSync('./src/pangeran.png'), thumbnail: require('fs').readFileSync('./src/pangeran.png'),sendEphemeral: true}}}
	conn.p[id] = [
	await conn.sendKontak(m.chat, kontak2, fkontak, { contexInfo: { forwardingScore: 99999, isForwarded: true } })
	]
	await delay(100)
  return conn.sendMessage(m.chat, { text: `*Hay Tante @${await m.sender.split('@')[0]}, Itu Nomor Ownerku Yang Paling Ganteng Tan, Jangan Di Jadikan Pelampiasanmu yah ^_^*`, mentions: [m.sender] }, { quoted: conn.p[id][0] })
  await delay(100)
